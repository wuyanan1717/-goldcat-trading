 ...
 */
export function detectDivergence(data: ChartData[]): 'BULLISH' | 'BEARISH' | 'NONE' {
 ...
 * Bullish Signal.
 */
export function detectFlatBottomGreen(data: ChartData[]): number[] {
 ...
 * Bearish Signal.
 * Criteria based on PDF:
 * 1. Trend: Downtrend or Reversal context (simplified to previous candles).
 * 2. Setup: Previous candle is a Doji (Cross Star).
 * 3. Trigger: Current candle is a Long Red Candle.
 * 4. Feature: No Upper Shadow (or very small) on the Red Candle.
 * 5. Action: Breaking down below Doji's support.
 */
export function detectBeheadingRed(data: ChartData[]): number[] {
  const indices: number[] = [];
  if (data.length < 2) return indices;
