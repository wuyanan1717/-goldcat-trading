...
export interface BacktestResult {
  totalSignals: number;
  longStats: { wins: number; losses: number; rate: number };
  shortStats: { wins: number; losses: number; rate: number };
  totalReturn: number;
  logs: string[];
}
...
export function runBacktestSimulation(
  ...
  const findCandle = (data: ChartData[], time: number) => {
    return data.find(d => d.time && d.time <= time && d.time + (getIntervalMs(data) - 1) >= time);
  };

  const getIntervalMs = (data: ChartData[]) => {
  ...
  const calculateRate = (w: number, l: number) => {
    const t = w + l;
