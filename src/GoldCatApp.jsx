...
function GoldCatApp() {
    ...
    useEffect(() => {
        if (user && user.email && isDataLoaded) {
            // Validation: Don't sync invalid values or empty strings
            if (totalCapital === '' || totalCapital === null || isNaN(Number(totalCapital))) {
                return;
            }

            // Save to both database and localStorage (Debounced)
    ...
    // 删除交易
    // 删除交易 - Step 1: Trigger Modal
    const removeTrade = (tradeId) => {
        setTradeToDelete(tradeId);
    };

    // 删除交易 - Step 2: Execute Logic
        ...
        <div className="fixed inset-0 bg-black text-gray-200 font-sans selection:bg-amber-500/30 notranslate overflow-auto flex flex-col min-h-screen" translate="no">
            ...
            */}
            ...
            <main className="w-full max-w-7xl mx-auto px-4 py-6">
                ...
                {((!user && !showGuestDashboard) || (user && explicitLandingView)) && (
                    <div className="relative min-h-[90vh] flex flex-col items-center justify-center text-center px-4 overflow-hidden bg-black">
                        ...
                        {/* Content Overlay */}
                        <div className="relative z-10 max-w-5xl mx-auto animate-in fade-in zoom-in duration-1000 slide-in-from-bottom-8 py-20 pointer-events-none">
            ...
            {/* Payment Modal */}
                ...
                {tradeToDelete && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-[#111] border border-gray-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl relative overflow-hidden">
                            {/* Glow effect */}
                            ...
                            <div className="flex gap-3 justify-end">
                                ...
                                <button
                                    onClick={executeDeleteTrade}
                                    className="px-4 py-2 bg-red-900/50 hover:bg-red-800 text-red-200 border border-red-900 rounded-lg transition-colors font-bold ml-2"
                                >
                                    {language === 'zh' ? '确认删除' : 'Delete Trade'}
                                </button>
                            </div>
                        </div>
                    </div>
