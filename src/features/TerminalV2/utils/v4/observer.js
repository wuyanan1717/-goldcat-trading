...
export async function consultObserver(
    ...
    // --- Context Preparation ---
    const currentPrice = (data1m.length > 0 ? data1m[data1m.length - 1].v : (data1h.length > 0 ? data1h[data1h.length - 1].v : 0));

    // Calculate 24h High/Low (approx based on available 1h data)
    let rangeHigh = currentPrice;
    let rangeLow = currentPrice;
    if (data1h.length > 0) {
        const recent24 = data1h.slice(-24);
        const highs = recent24.map(d => d.h || d.v);
        const lows = recent24.map(d => d.l || d.v);
    ...
    **24h Range: $${rangeLow} - $${rangeHigh}**
    ...
    即使数据不够完美，你也**必须**基于现有 1H/5M 数据估算一个最合理的【下方支撑位】和【上方阻力位】价格。
    **特别注意：支撑/阻力位必须围绕当前价格 ($${currentPrice}) 展开，不能偏离太远。** 
    绝对不能返回 0 或 null。
    ...
    必须返回标准 JSON 格式，不包含 markdown 代码块：
    {
      ...
      "support_price": number (必须返回数值。基于当前价格 $${currentPrice} 估算的下方关键支撑位，不能为0),
      "resistance_price": number (必须返回数值。基于当前价格 $${currentPrice} 估算的上方关键阻力位，不能为0)
    }
  `;
    ...
    **24h Range: $${rangeLow} - $${rangeHigh}**
    ...
    You **MUST** estimate a reasonable [Support Level] and [Resistance Level] based on the provided 1H/5M data.
    **CRITICAL: Support/Resistance MUST be relevant to the Current Price ($${currentPrice}).** 
    ...
    Must return standard JSON format, no markdown code blocks:
    {
      ...
      "support_price": number (MUST BE A NUMBER > 0. Estimated key support level around $${currentPrice}),
      "resistance_price": number (MUST BE A NUMBER > 0. Estimated key resistance level around $${currentPrice})
    }
  `;

    const promptText = isEn
        ? `${systemInstructionEN}\n\nAnalyze ${symbol}'s quantum state immediately (Price: $${currentPrice}) and output JSON.`
        : `${systemInstructionZH}\n\n立即分析 ${symbol} 的量子场态 (当前价: $${currentPrice}) 并输出 JSON。`;
    ...
    try {
        ...
        // Fallback: Automatic Calculation from 1H Data (Recent 24 Candles)
        // This ensures "real values" are shown even if AI returns 0
        if ((support === 0 || resistance === 0) && data1h && data1h.length > 0) {
            try {
                const recent = data1h.slice(-24);
                const lows = recent.map(c => c.l || c.v || 0);
                const highs = recent.map(c => c.h || c.v || 0);

                if (support === 0) support = Math.min(...lows);
                if (resistance === 0) resistance = Math.max(...highs);
            } catch (calcErr) {
                console.error("Local S/R Calc Failed", calcErr);
            }
        }
