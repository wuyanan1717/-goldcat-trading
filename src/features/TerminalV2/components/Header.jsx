...
export const Header = ({
    ...
    return (
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-3 mb-2 gap-4">
            <div>
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <button
                    ...
                    className={`px-3 py-2 text-xs border rounded-sm uppercase transition-all flex items-center gap-2 ${isTacticalEnabled
                ...
                >
                    ...
                    {lang === 'en' ? 'Reset Stream' : '重置数据流'}
                </button>
                <button
                    onClick={onScan}
                    disabled={isScanning}
                    className={`px-6 py-2 text-sm font-bold text-black rounded-sm shadow-[0_0_20px_rgba(234,179,8,0.4)] transition-all flex items-center gap-2 ${isScanning ? 'bg-yellow-600 cursor-not-allowed opacity-80' : 'bg-yellow-500 hover:bg-yellow-400 hover:scale-105'
                        }`}
                >
                    {isScanning ? (
                        <>
                            <RefreshCw className="w-4 h-4 animate-spin" />
                            {lang === 'en' ? 'Scanning...' : '观测中...'}
                        </>
                    ) : (
                        <>
                            <Eye className="w-4 h-4" />
                            {lang === 'en' ? 'Start Scan' : '启动观测'}
                        </>
                    )}
                </button>
            </div>
